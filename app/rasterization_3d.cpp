    ...
    Matrix<Pixel>& output_img)
{
    ...
    for(size_t prim_idx = 0; prim_idx < index_buffer.shape(1); prim_idx++)
    {
        ...
        for(size_t i = 0; i < 3; i++)
        {
            ...
            Matrix<float> vertex = vertex_buffer(Block({}, {vtx_idx, vtx_idx+1}));
            aabb.extend(vertex);
            prim(Block({},{i, i+1})) = vertex;
            prim_texture_coord(Block({},{i, i+1})) = texture_coord_buffer(Block({}, {vtx_idx, vtx_idx + 1}));
        }
        Vector<float> extent = aabb.max() - aabb.min();
        std::vector<float> sample_pts_data;
        sample_pts_data.reserve(size_t(extent(0) * extent(1) + 1));
        
        for(size_t i = size_t(aabb.min()(0)); i < aabb.max()(0) + 1; i++)
        {
            for(size_t j = size_t(aabb.min()(1)); j < aabb.max()(1) + 1; j++)
            {
        ...
        for(size_t i = 0; i < sample_pts.shape(1); i++)
        {
            ...
            output_img(size_t(sample_pts(0, i)), size_t(sample_pts(1, i))) = colors(i, 0);
        }
...
{
    Shape output_reso{300, 300};
    Vector<size_t> ncube_resolution{2,2};

    Matrix<Pixel> texture({2,2}, {Pixel::red(), Pixel::green(), Pixel::blue(), Pixel::black()});
    

    auto vertices = generateNCubeVertices({100.f, 100.f}, ncube_resolution);

    vertices += Vector<float>{50,50};
    ...
    fragmentShading(vertices, indices, texture, texture_coord_buffer, img_out);
