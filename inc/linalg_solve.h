...
{
template<typename DType>
Vector<DType> project(const Vector<DType>& u, const Vector<DType>& a)
{
    Vector<DType> u_dir = u.normalized();
    return u_dir.dot(a) * u_dir;
}
...
Matrix<DType> calcMatQFromReflection(const Matrix<DType>& mat)
{
    ...
    {
        Vector<DType> a_i(mat(Col(i)));
        mat_u(Col(i)) = a_i;
        for(size_t j = 0; j < i; j++)
        {
            Vector<DType> proj(project(mat_u(Col(j)).asVector(), a_i));
